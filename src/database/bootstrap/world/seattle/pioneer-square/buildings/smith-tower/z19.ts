  ...
  rooms: {
    ...
    '%EL_z19': {
      ...
      exits: {
        ...
        south: { room: '%C2_z19', door: '%D_C2_z19' },
        in: '%E',
      },
      methods: {
    ...
    '%A1_z19': {
      ...
      methods: {
        ...
        onContentLeaving: "const obj = args[0];\nif (obj?.unregisterVerbsFrom) {\n  await obj.unregisterVerbsFrom(self.id);\n}",
        lookOutWindow: "const player = args[0];\nreturn 'From this height above Pioneer Square, you catch the street below: scooters weaving between delivery vans, bus brakes sighing, late-night sirens, and pedestrians threading past food carts. The wind rattles the glass as you watch the city shift far beneath you (floor 19).';",
    ...
    '%C2_z19': {
      ...
      x: 0, y: 0, z: 19,
      exits: {\"north\":{\"room\":\"%EL_z19\",\"door\":\"%D_C2_z19\"},\"east\":\"%C3_z19\"},
      methods: {
        onContentArrived: "const obj = args[0];\nif (!obj?.registerVerb) return;\nawait obj.registerVerb(['look out window', 'look through window', 'gaze out window', 'look down'], self, 'lookOutWindow');",
        onContentLeaving: "const obj = args[0];\nif (obj?.unregisterVerbsFrom) {\n  await obj.unregisterVerbsFrom(self.id);\n}",
        lookOutWindow: "const player = args[0];\nreturn 'From this height above Pioneer Square, you catch the street below: scooters weaving between delivery vans, bus brakes sighing, late-night sirens, and pedestrians threading past food carts. The wind rattles the glass as you watch the city shift far beneath you (floor 19).';",
